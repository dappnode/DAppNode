...
git commit -m "Update manifest and docker-compose versions" && git push origin || echo "Files are already updated"
...
git add "dappnode_package.json"
git add "docker-compose.yml"
git commit -m "Advance manifest and docker-compose versions to new version"
